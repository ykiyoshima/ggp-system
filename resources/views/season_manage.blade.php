...
<body>
    <h1 class="text-6xl text-gray-900 font-bold text-center pt-32 pb-16">シーズン管理</h1>
    <a id="menu_btn" href="/admin_menu" class="block w-64 py-4 mb-16 mx-auto rounded text-2xl text-center bg-gray-300 text-gray-900 hover:bg-gray-500 hover:text-gray-100">メニューに戻る</a>
    <a id="season_make_btn" href="/season_make" class="block w-64 py-4 mb-16 mx-auto rounded text-2xl text-center bg-gray-300 text-gray-900 hover:bg-gray-500 hover:text-gray-100">シーズン作成</a>
    <div>
      ...
      @if ($season->season_date >= date('Y-m-d'))
        <div class="flex justify-center text-xl text-gray-900 text-center mb-4">
          ...
          <div class="w-28 flex justify-center items-center border-y border-r border-gray-900">
            <form action="{{route('season.destroy', $season->id)}}" method="POST">
              @method('delete')
              @csrf
              <button class="px-4 py-1 text-base mx-auto rounded bg-gray-300 hover:bg-gray-500 hover:text-gray-100 cursor-pointer">削除</button>
            </form>
          </div>
        </div>
      @endif
